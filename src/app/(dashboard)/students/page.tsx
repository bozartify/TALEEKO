...
import {
  ...
  Activity, Shield, Zap, ArrowUpRight, Layers, RefreshCw
} from 'lucide-react'
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-surface-400">{students.length} students · AP Biology class · {avgAttend}% avg attendance</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => setGroupOpen(o => !o)} className="btn-secondary text-xs px-3 py-1.5">
      ...
      <FadeUp delay={0.05}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: 'At Risk',        value: atRisk + needsAttn,     icon: AlertTriangle, color: '#ef4444', sub: `${totalMissing} missing assignments` },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
      ...
      <FadeUp delay={0.07}>
        <div className="glass-card overflow-hidden border border-accent-500/15">
          <button className="w-full flex items-center justify-between px-5 py-4 hover:bg-white/[0.02] transition-colors" onClick={() => setAiInsightOpen(o => !o)}>
              ...
              <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                <div className="px-5 pb-4 border-t border-white/[0.06] pt-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                    {[
                      { color: '#ef4444', title: 'Mason Kim – Immediate Action', desc: 'Missing 3 assignments + 82% attendance. IEP flag active. Parent contact within 24h recommended.' },
                      { color: '#f59e0b', title: 'Sofia Rodriguez – ELL Support', desc: 'Lab report scores declining. Suggest differentiated materials and ELL resource packet for next unit.' },
                      { color: '#10b981', title: 'Noah Thompson – Accelerate', desc: 'Consistently 95%+ across all work. Honors acceleration or peer tutor role recommended immediately.' },
                    ].map((insight, i) => (
                      <div key={insight.title} className="p-3.5 rounded-2xl border" style={{ borderColor: insight.color + '30', backgroundColor: insight.color + '08' }}>
                        <div className="w-1.5 h-1.5 rounded-full mb-2" style={{ backgroundColor: insight.color }} />
                  ...
                  <div className="flex items-center gap-2">
                    <button className="text-xs text-accent-400 hover:text-accent-300 font-semibold">View Full Action Plan</button>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
            <div className="glass-card p-5 border border-neon-400/15">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-sm font-bold text-white flex items-center gap-2"><Layers className="w-4 h-4 text-neon-400" /> AI Group Builder</h4>
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="btn-secondary text-xs px-3 py-1.5"><Download className="w-3.5 h-3.5" /> Export Group List</button>
                <button className="btn-secondary text-xs px-3 py-1.5"><RefreshCw className="w-3.5 h-3.5" /> Regenerate</button>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {(['avg', 'attendance', 'streak', 'name'] as const).map(k => (
                  <button key={k} onClick={() => setSortKey(k)} className={`text-xs px-2 py-1 rounded-lg transition-all ${sortKey === k ? 'bg-white/[0.08] text-white' : 'text-surface-500 hover:text-surface-300'}`}>
          ...
          <div className="space-y-2">
            {filtered.map((student, i) => {
              ...
              return (
                <motion.div
                  key={student.id}
                  className={`glass-card p-4 cursor-pointer transition-all ${isSelected ? 'border-accent-500/30 shadow-[0_0_20px_rgba(99,102,241,0.08)]' : ''}`}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        <div className="mt-4 pt-4 border-t border-white/[0.06]">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <button className="btn-gradient text-xs px-3 py-1.5 ml-auto"><Sparkles className="w-3 h-3" /> AI Action Plan</button>
                          </div>
                        </div>
                      </motion.div>
                    )}
        ...
        <div className="space-y-4">
                ...
                {[
                  ...
                  <div key={tier.range}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-bold" style={{ color: tier.color }}>{tier.count} students</span>
                    </div>
          ...
          <FadeInWhenVisible delay={0.22}>
            <div className="glass-card p-5">
              <h4 className="text-sm font-bold text-white mb-3 flex items-center gap-2"><Activity className="w-4 h-4 text-electric-400" /> Class Health Score</h4>
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-surface-400 leading-relaxed">Class is performing well. 3 students need intervention to bring overall score above 85.</p>
                </div>
              </div>
              <div className="space-y-1.5">
                {[
          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div className="relative glass-card p-6 w-full max-w-md z-10" initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button className="text-[10px] text-accent-400 flex items-center gap-1"><Sparkles className="w-3 h-3" /> AI Draft</button>
                  </div>
              ...
              <div className="flex gap-3 mt-5">
                <button className="btn-secondary text-xs flex-1">Schedule Send</button>
                <motion.button className="btn-gradient text-xs flex-1" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setMessageModal(null)}>
            ...
            <motion.div className="relative glass-card p-6 w-full max-w-md z-10" initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.92, opacity: 0 }}>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Student Email', placeholder: 'jane.s@student.edu' },
                  { label: 'Parent Email', placeholder: 'parent@example.com' },
                ].map(f => (
                  <div key={f.label}>
                    <label className="block text-xs font-semibold text-surface-300 mb-1.5">{f.label}</label>
                    <input type="text" placeholder={f.placeholder} className="w-full bg-white/[0.04] border border-white/[0.08] text-surface-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500" />
                  </div>
                ))}
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" className="w-3.5 h-3.5 accent-accent-500 rounded" />
              ...
              <div className="flex gap-3 mt-5">
                ...
                <motion.button className="btn-gradient text-xs flex-1" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setAddStudentModal(false)}>
                  <UserPlus className="w-3.5 h-3.5" /> Add Student
                </motion.button>
              </div>
            </motion.div>

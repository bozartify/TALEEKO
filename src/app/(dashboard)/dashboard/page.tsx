...
import {
  ...
  ChevronRight, TrendingUp, Users, Clock, Star
} from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {tools.map((tool) => (
            <Link key={tool.label} href={tool.href} className="tool-card group">
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
          {recentLessons.map((lesson, i) => (
            <div key={lesson.title} className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors cursor-pointer ${i < recentLessons.length - 1 ? 'border-b border-slate-100' : ''}`}>

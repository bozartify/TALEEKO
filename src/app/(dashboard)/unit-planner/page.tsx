...
import {
  ...
  Calendar, BookOpen, ClipboardList, Layers, ChevronRight, Sparkles,
} from 'lucide-react'
...
export default function UnitPlannerPage() {
  ...
  async function handleGenerateUnit(unit: Unit) {
    ...
    try {
      const dur = unit.weekEnd - unit.weekStart + 1
      const prompt = `Create a detailed ${dur}-week ${unit.subject} unit plan for ${unit.grade} titled "${unit.title}". Return ONLY valid JSON: {"objectives":["obj1","obj2","obj3"],"assessments":[{"title":"Assessment Name","type":"Quiz"}],"timeline":[{"week":1,"topic":"Week 1 Topic"}]} with ${dur} timeline entries, 3-5 objectives, and 2-3 assessments. Types: Quiz, Project, Essay, Lab, Presentation, Exit Ticket.`
      ...
      const data = JSON.parse(jsonMatch[0]) as { objectives: string[]; assessments: { title: string; type: string }[]; timeline: { week: number; topic: string }[] }
      const updater = (u: Unit) => u.id !== unit.id ? u : {
        ...u,
      ...
      setSelectedUnit(prev => prev?.id === unit.id ? updater(prev) : prev)
      showToast('Unit plan generated by AI!')
    } catch {
        ...
        <FadeInWhenVisible delay={0.15}>
          <div id="quick-add-form" className="glass-card p-5 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <motion.button
                className="btn-gradient flex-1 text-sm justify-center"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.97 }}
                onClick={() => {
                  if (!form.title.trim()) { showToast('Please enter a unit title'); return }
                  const dur = parseInt(form.durationWeeks) || 2
                  const lastWeekEnd = Math.max(...units.map(u => u.weekEnd), 0)
                  const unit: Unit = { id: `u${Date.now()}`, title: form.title.trim(), subject: form.subject, grade: form.grade.trim() || 'Unspecified', color: SUBJECT_COLORS[form.subject] ?? '#6366f1', weekStart: lastWeekEnd + 1, weekEnd: lastWeekEnd + dur, standardsCount: parseInt(form.standards) || 0, status: 'Planning', description: `A new ${form.subject} unit spanning ${dur} week${dur !== 1 ? 's' : ''}.`, progress: 0, objectives: [], assessments: [], timeline: Array.from({ length: dur }, (_, i) => ({ week: lastWeekEnd + 1 + i, topic: `Week ${i + 1} — to be planned` })) }
                  setUnits(prev => { const next = [...prev, unit]; try { localStorage.setItem('taleeko_units', JSON.stringify(next)) } catch {} return next })
                  setForm({ title: '', subject: 'Biology', grade: '', durationWeeks: '', standards: '' })
                  handleGenerateUnit(unit)
                }}
              >
                ...
                <section>
                  ...
                  {selectedUnit.objectives.length > 0 ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-sm text-surface-500 italic">No objectives yet.</p>
                      <button className="btn-gradient text-xs px-3 py-1.5" disabled={generatingUnit === selectedUnit.id} onClick={() => handleGenerateUnit(selectedUnit)}>
                        {generatingUnit === selectedUnit.id ? <><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin inline-block" />&nbsp;Planning…</> : <><Sparkles className="w-3.5 h-3.5" />AI Generate Plan</>}
                      </button>
                    </div>
                  )}
                </section>

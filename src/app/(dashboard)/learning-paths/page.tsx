...
export default function LearningPathsPage() {
  ...
  async function handleGeneratePath(path: LearningPath) {
    setGenerating(path.id)
    showToast('AI is building your learning path steps…')
    try {
      const prompt = `Generate a ${path.estimatedWeeks}-week learning path for ${path.subject} (${path.grade}) titled "${path.title}". ${path.description ? `Context: ${path.description}. ` : ''}Create 5-7 sequential steps. Return ONLY valid JSON: {"steps":[{"id":"s1","title":"Step Title","type":"lesson","duration":30}]} where type is one of: lesson, quiz, activity, video, discussion. duration is minutes (10-60).`
      ...
      while (true) {
        ...
        for (const line of chunk.split('\n')) {
          ...
          try { const parsed = JSON.parse(trimmed); if (parsed.type === 'text') fullText += parsed.text } catch { /* skip */ }
        }
      }
      const jsonMatch = fullText.match(/\{[\s\S]*\}/)
      if (!jsonMatch) throw new Error('No JSON')
      const { steps } = JSON.parse(jsonMatch[0]) as { steps: PathStep[] }
      setPaths(prev => prev.map(p => p.id === path.id ? { ...p, steps: steps.map((s, i) => ({ ...s, id: `s${i+1}`, completed: false })) } : p))
      setSelectedPath(prev => prev?.id === path.id ? { ...prev, steps: steps.map((s, i) => ({ ...s, id: `s${i+1}`, completed: false })) } : prev)
      showToast(`${steps.length} steps generated by AI!`)
    } catch {
  ...
  function handleCreatePath(withAI = false) {
                ...
                <div className="space-y-2">
                  ...
                  {selectedPath.steps.length === 0 && (
                    <div className="glass-card p-6 text-center space-y-3">
                      <p className="text-surface-500 text-xs">No steps yet — let AI build this path for you.</p>
                      <button className="btn-gradient text-xs px-4 py-2 mx-auto" disabled={generating === selectedPath.id} onClick={() => handleGeneratePath(selectedPath)}>
                        {generating === selectedPath.id ? <><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />&nbsp;Generating…</> : <><Sparkles className="w-3.5 h-3.5" />AI Generate Steps</>}
                      </button>
                    </div>
                  )}
                </div>
          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div className="relative glass-card p-6 w-full max-w-sm z-10 space-y-4" initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }}>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={() => handleCreatePath(false)} className="btn-secondary text-xs px-4 py-2">
                  <Plus className="w-3 h-3" /> Create
                </button>
                <button onClick={() => handleCreatePath(true)} className="btn-gradient text-xs px-4 py-2">
                  <Sparkles className="w-3 h-3" /> Create + AI Steps
                </button>
              </div>
            </motion.div>
          </motion.div>

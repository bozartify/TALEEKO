...
export default function AnalysisCard({ data }: Props) {
  ...
  return (
    <div className="w-full max-w-2xl bg-white rounded-2xl border border-sky-100 shadow-card overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-sky-50" style={{ background: 'linear-gradient(135deg,rgba(14,165,233,0.08),rgba(2,132,199,0.04))' }}>
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-sky-500 flex items-center justify-center">
            <BarChart2 className="w-4 h-4 text-white" />
          </div>
          <div>
            <p className="text-xs font-semibold text-sky-600 uppercase tracking-wider">Lesson Analysis</p>
            <h3 className="font-black text-slate-900 text-sm">AI Feedback Report</h3>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {data.overallScore && (
          <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
            ...
            <div>
              <p className="text-sm font-bold text-slate-900">Overall Score</p>
        ...
        {strengths.length > 0 && (
          <div>
            ...
            <ul className="space-y-1">
              {strengths.map((s, i) => <li key={i} className="text-xs text-slate-600 pl-5 list-disc">{s}</li>)}
            </ul>
          </div>
        )}
        ...
        {improvements.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-xs font-bold text-slate-700">Areas to Improve</p>
            </div>
            <ul className="space-y-1">
              {improvements.map((s, i) => <li key={i} className="text-xs text-slate-600 pl-5 list-disc">{s}</li>)}
            </ul>
          </div>
        )}
        ...
        {recommendations.length > 0 && (
          <div className="p-3 bg-brand-50 rounded-xl">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-xs font-bold text-brand-700">Recommendations</p>
            </div>
            <ol className="space-y-1">
              {recommendations.map((r, i) => (
                <li key={i} className="flex gap-2">
                  <span className="text-brand-400 font-bold text-xs flex-shrink-0">{i + 1}.</span>
